        ...
        function (e) {
          ...
          if (buttonOpen) {
            ...
            if (this._dataValue !== "error") {
              ...
              this.popupLogging(
                `Не заполнен атрибут у ${buttonOpen.classList}`
              );
            return;
          }
    ...
    open(selectorValue) {
      if (bodyLockStatus) {
        ...
        if (this.targetOpen.element) {
          ...
          this.popupLogging(
            `Такого попапа нету. Проверьте корректность введения.`
          );
      }
    }
    close(selectorValue) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function processJsonData(data) {
      ...
      data.forEach(function (item) {
        ...
        flagButton.className = item.flag
        ...
        editButton.addEventListener("click", function () {
          ...
          Promise.all([
            fetchOptions(`${urlServer}cargo_heads`),
            fetchOptions(`${urlServer}cargo_trailers`),
          ]).then(([headOptions, trailerOptions]) => {
              ...
              if (
                ...
                if (cell.getAttribute("data-id") === "headNumber") {
                  ...
                  headOptions.forEach(function (optionValue) {
                    ...
                    if (optionValue.headNumber === cell.textContent)
                  ...
                  trailerOptions.forEach(function (optionValue) {
                    ...
                    if (optionValue.trailerNumber === cell.textContent)
                      option.selected = true;
                    input.appendChild(option);
                  });
                } else {
                  ...
                  input.type =
                    cell.getAttribute("data-id") === "date" ? "date" : "text";
        ...
        saveButton.addEventListener("click", function () {
          ...
          row.querySelectorAll(".list__value").forEach(function (cell) {
            if (cell.querySelector("input") || cell.querySelector("select")) {
              ...
              if (cell.getAttribute("data-id") === "date")
                cell.textContent = input.value.replace(
                  /(\d{4})-(\d{2})-(\d{2})/,
                  "$3.$2.$1"
                );
      ...
      function filterRows() {
        const headFilter = document
          .getElementById("searchHeadNumber")
          .value.toLowerCase();
        const trailerFilter = document
          .getElementById("searchTrailerNumber")
          .value.toLowerCase();
        const dateFilter = document.getElementById("searchDate").value;
        const rows = document.querySelectorAll(".list__item");
        rows.forEach((row) => {
          const headNumber = row
            .querySelector("[data-id='headNumber']")
            .textContent.toLowerCase();
          const trailerNumber = row
            .querySelector("[data-id='trailerNumber']")
            .textContent.toLowerCase();
          const date = row
            .querySelector("[data-id='date']")
            .textContent.replace(/(\d{2})\.(\d{2})\.(\d{4})/, "$3-$2-$1");
          if (
            headNumber.includes(headFilter) &&
            trailerNumber.includes(trailerFilter) &&
    ...
    function jsonDataTrailers() {
      fetch(`${urlServer}cargo_trailers`)
        .then((response) => {
          if (!response.ok)
            throw new Error(
              "Network response was not ok " + response.statusText
